# This module provides a lightweight evaluation harness (`Tester` / `evaluate`) to benchmark and audit model predictions 
# against ground truth pricing data. It handles string extraction, statistical performance tracking, and interactive visualization.

# # Core Capabilities
# * Output Normalization & Parsing: Cleans currency strings and robustly extracts numeric scalar values using regex.
# * Dynamic Error Stratification: Color-codes predictions (Green / Orange / Red) based on absolute and relative percentage thresholds.
# * Statistical Auditing: Calculates Mean Absolute Error (MAE), Mean Squared Error (MSE), Coefficient of Determination ($R^2$), and running 95% Confidence Intervals.
# * Interactive Diagnostics: Generates Plotly-powered scatter plots (Actual vs. Predicted with $y=x$ parity reference) and cumulative error convergence trends.
